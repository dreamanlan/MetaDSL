...
namespace Dsl.Parser
{
    struct LuaError
    {
        ...
        {
            mLog.Error("[error] syntax error: skipping input {0}, last token {1} line {2}, cur token {3} line {4} [production: {5} nonterminal: {6} ]", LuaString.GetSymbolName(token), luaToken.getLastToken(), luaToken.getLastLineNumber(), luaToken.getCurToken(), luaToken.getLineNumber(), LuaString.GetProductionName(productionNumber), LuaString.GetSymbolName(nonterminal));

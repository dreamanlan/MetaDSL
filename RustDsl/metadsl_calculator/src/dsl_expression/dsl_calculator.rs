...
use metadsl::dsl::{DslLogDelegationBox, ISyntaxComponent, SyntaxComponent};
use metadsl_macros::{
...
pub type LoadValueFailbackDelegation<'a> =
    dyn Fn(&ValueData, &Rc<DslCalculatorCell<'a>>) -> Option<ExpressionBox<'a>>;
pub type LoadFunctionFailbackDelegation<'a> =
    dyn Fn(&FunctionData, &Rc<DslCalculatorCell<'a>>) -> Option<ExpressionBox<'a>>;
pub type LoadStatementFailbackDelegation<'a> =
    dyn Fn(&StatementData, &Rc<DslCalculatorCell<'a>>) -> Option<ExpressionBox<'a>>;
...
pub fn create_expression_factory<'a, T>() -> ExpressionFactoryBox<'a>
where
    T: IExpression<'a> + Default + 'a,
{
    return Box::new(|| Box::new(T::default()));
}
pub fn create_class_factory<'a, T>() -> ObjectFactoryBox<'a>
where
    T: IObjectDispatch<'a> + Default + 'a,
{
...
pub trait IObjectDispatch<'a> {
    ...
    fn invoke_method(
        &mut self,
        disp_id: u32,
        args: &Vec<&DslCalculatorValue>,
    ) -> Option<DslCalculatorValue>;
}
pub trait IExpression<'a> {
    fn calc(&mut self) -> DslCalculatorValue;
    fn load_syntax_component(
        &mut self,
        dsl: &SyntaxComponent,
        calculator: Rc<DslCalculatorCell<'a>>,
    ) -> bool;
    fn load_value_or_function(
        &mut self,
        dsl: &ValueOrFunction,
        calculator: Rc<DslCalculatorCell<'a>>,
    ) -> bool;
    fn load_value_syntax(&mut self, dsl: ValueData, calculator: Rc<DslCalculatorCell<'a>>) -> bool;
    fn load_function_syntax(
        &mut self,
        dsl: FunctionData,
        calculator: Rc<DslCalculatorCell<'a>>,
    ) -> bool;
    fn load_statement_syntax(
        &mut self,
        dsl: StatementData,
        calculator: Rc<DslCalculatorCell<'a>>,
    ) -> bool;
...
pub trait AbstractExpression<'a>: IExpression<'a> {
    ...
    fn load_syntax_component(
        ...
        match dsl {
            ...
            SyntaxComponent::Function(func_data) => {
                return AbstractExpression::load_function_syntax(
                    self,
                    func_data.clone(),
                    calculator,
                );
            ...
            SyntaxComponent::Statement(statement_data) => {
                return AbstractExpression::load_statement_syntax(
                    self,
                    statement_data.clone(),
                    calculator,
                );
            }
        }
                ...
                if let SyntaxComponent::Function(owned_func_data) = self.syntax_component() {
                    if let Some(ps) = owned_func_data.params() {
                        for param in ps.iter() {
                            if let Some(syn) =
                                DslCalculator::load_syntax_component(self.calculator(), param)
                            {
                                args.push(syn);
                            }
                        }
    ...
    fn to_string(&self) -> String {
        return format!(
            ...
            self.syntax_component().get_line()
        );
    }
    fn load_value(&mut self) -> bool {
        return false;
    }
    fn load_expressions(&mut self, _exps: &mut Vec<ExpressionBox<'a>>) -> bool {
        return false;
    }
...
pub trait SimpleExpressionBase<'a>: AbstractExpression<'a> {
    fn do_calc(&mut self) -> DslCalculatorValue {
        ...
        self.calculator()
            .borrow_mut()
            .recycle_calculator_value_list(operands);
        return r;
    }
...
impl<'a> AbstractExpression<'a> for GlobalVarSet<'a> {
    fn do_calc(&mut self) -> DslCalculatorValue {
        ...
        if self.m_var_ix < std::i32::MAX {
            self.calculator()
                .borrow_mut()
                .set_global_varaible_by_index(self.m_var_ix, v.clone());
        } else if let Some(var_id) = &self.m_var_id {
            if var_id.len() > 0 {
                let ix = self
                    .calculator()
                    .borrow_mut()
                    .alloc_global_variable_index(&var_id);
                self.m_var_ix = ix;
                if self.m_var_ix < std::i32::MAX {
                    self.calculator()
                        .borrow_mut()
                    ...
                    self.calculator()
                        .borrow_mut()
                        .set_global_variable(&var_id, v.clone());
                }
            }
        }
        return v;
...
impl<'a> AbstractExpression<'a> for GlobalVarGet<'a> {
    fn do_calc(&mut self) -> DslCalculatorValue {
        ...
        if is_break {
            self.calculator()
                .borrow_mut()
                .set_run_state(RunStateEnum::Break);
            return ret;
        } else if is_continue {
            self.calculator()
                .borrow_mut()
                .set_run_state(RunStateEnum::Continue);
            return ret;
        } else if self.m_var_ix < std::i32::MAX {
            if let Some(v) = self
                .calculator()
                .borrow()
                .get_global_varaible_by_index(self.m_var_ix)
            {
                ret = v.clone();
            }
        } else if let Some(var_id) = &self.m_var_id {
            ...
            if self.m_var_ix < std::i32::MAX {
                ...
                self.calculator()
                    .borrow()
                    .error(&format!("unassigned global var '{0}'", var_id));
            }
        }
        return ret;
    }
...
impl<'a> AbstractExpression<'a> for LocalVarSet<'a> {
    fn do_calc(&mut self) -> DslCalculatorValue {
        ...
        if self.m_var_ix < std::i32::MAX {
            self.calculator()
                .borrow_mut()
                .set_local_varaible_by_index(self.m_var_ix, v.clone());
        } else {
            ...
            if let Some(var_id) = &self.m_var_id {
                var_ix = self
                    .calculator()
                    .borrow_mut()
                    .alloc_local_variable_index(var_id);
            }
            if var_ix < std::i32::MAX {
                self.m_var_ix = var_ix;
                self.calculator()
                    .borrow_mut()
                    .set_local_varaible_by_index(var_ix, v.clone());
            } else {
                if let Some(var_id) = &self.m_var_id {
                    self.calculator()
                        .borrow()
                        .error(&format!("unassigned local var '{0}'", var_id));
                }
            }
        }
        return v;
    ...
    fn do_calc(&mut self) -> DslCalculatorValue {
        ...
        if self.m_var_ix < std::i32::MAX {
            if let Some(r) = self
                .calculator()
                .borrow()
                .get_local_varaible_by_index(self.m_var_ix)
            {
                ret = r.clone();
            }
        } else {
            ...
            if var_ix < std::i32::MAX {
                ...
                if let Some(r) = self
                    .calculator()
                    .borrow()
                    .get_local_varaible_by_index(var_ix)
                {
                    ret = r.clone();
                }
            } else {
            ...
            while ix < num && ix < MAX_TUPLE_ELEMENT_NUM {
                ...
                match ix {
                    ...
                    6 => {
                        v7 = exp.calc();
                        if num == 7 {
                            return DslCalculatorValue::Tuple7(Box::new((
                    ...
                    7 => {
                        v8 = exp.calc();
                        if num == 8 {
                            return DslCalculatorValue::Tuple8(Box::new((
                    ...
                    8 => {
                        if num == 8 {
                            let v9 = exp.calc();
                            return DslCalculatorValue::Tuple9(Box::new((
                                v1, v2, v3, v4, v5, v6, v7, v8, v9,
                            )));
                        } else {
                            let tuple = self.pack_values(start + 8);
                            return DslCalculatorValue::Tuple9(Box::new((
                                v1, v2, v3, v4, v5, v6, v7, v8, tuple,
                            )));
                        }
                    }
                    _ => {}
...
impl<'a> TupleSetExp<'a> {
                ...
                match p {
                    ...
                    SyntaxComponent::Function(func_data) => {
                        if func_data.get_param_num() == 1 {
                            ...
                            if let Some(embedded_vars) = &self.m_embedded_vars {
                                ...
                                self.load_recursively(
                                    func_data,
                                    &mut embedded_vars.borrow_mut()[index],
                                );
                            }
                        }
                    }
                    _ => {
                        self.calculator().borrow().error(&format!(
                            "invalid tuple member {}. code:{} line:{}",
                            i,
                            p.to_script_string(false, &dsl::DEFAULT_DELIM),
                            p.get_line()
                        ));
                    }
                }
    ...
    fn match_recursively(
        &self,
        success: &mut bool,
        set_vars: &mut HashMap<String, DslCalculatorValue>,
        val: &DslCalculatorValue,
        var_ids: &Vec<(String, usize)>,
        start: usize,
    ) {
...
impl<'a> FuncInfo<'a> {
    ...
    pub fn add_param_name_index(&self, param_name: &str) {
        ...
        self.local_var_indexes
            .borrow_mut()
            .insert(String::from(param_name), -1 - ix);
    }
...
impl<'a> DslCalculator<'a> {
    ...
    pub fn init(&mut self) {
        ...
        self.register_api(
            "argnum",
            "argnum() api",
            create_expression_factory::<ArgNumGet>(),
        );
        ...
        self.register_api(
            "&",
            "bitand operator",
            create_expression_factory::<BitAndExp>(),
        );
        self.register_api(
            "|",
            "bitor operator",
            create_expression_factory::<BitOrExp>(),
        );
        self.register_api(
            "^",
            "bitxor operator",
            create_expression_factory::<BitXorExp>(),
        );
        self.register_api(
            "~",
            "bitnot operator",
            create_expression_factory::<BitNotExp>(),
        );
        self.register_api(
            "<<",
            "left shift operator",
            create_expression_factory::<LShiftExp>(),
        );
        self.register_api(
            ">>",
            "right shift operator",
            create_expression_factory::<RShiftExp>(),
        );
        self.register_api(
            ">",
            "great operator",
            create_expression_factory::<GreatExp>(),
        );
        self.register_api(
            ">=",
            "great equal operator",
            create_expression_factory::<GreatEqualExp>(),
        );
        self.register_api("<", "less operator", create_expression_factory::<LessExp>());
        self.register_api(
            "<=",
            "less equal operator",
            create_expression_factory::<LessEqualExp>(),
        );
        self.register_api(
            "==",
            "equal operator",
            create_expression_factory::<EqualExp>(),
        );
        self.register_api(
            "!=",
            "not equal operator",
            create_expression_factory::<NotEqualExp>(),
        );
        self.register_api(
            "&&",
            "logical and operator",
            create_expression_factory::<AndExp>(),
        );
        self.register_api(
            "||",
            "logical or operator",
            create_expression_factory::<OrExp>(),
        );
        self.register_api(
            "!",
            "logical not operator",
            create_expression_factory::<NotExp>(),
        );
        self.register_api(
            "?",
            "conditional expression",
            create_expression_factory::<CondExp>(),
        );

        self.register_api(
            "if",
            "if(cond)func(args); or if(cond){...}[elseif/elif(cond){...}else{...}]; statement",
            create_expression_factory::<IfExp>(),
        );
        self.register_api(
            "while",
            "while(cond)func(args); or while(cond){...}; statement, iterator is $$",
            create_expression_factory::<WhileExp>(),
        );
        self.register_api(
            "loop",
            "loop(ct)func(args); or loop(ct){...}; statement, iterator is $$",
            create_expression_factory::<LoopExp>(),
        );
        self.register_api(
            "looplist",
            "looplist(list)func(args); or looplist(list){...}; statement, iterator is $$",
            create_expression_factory::<LoopListExp>(),
        );
        self.register_api(
            "foreachvalue",
            "foreachvalue(args)func(args); or foreachvalue(args){...}; statement, iterator is $$",
            create_expression_factory::<ForEachValueExp>(),
        );

        self.register_api(
            "tuple",
            "(v1,v2,...) or tuple(v1,v2,...) object",
            create_expression_factory::<TupleExp>(),
        );
        self.register_api(
            "tupleset",
            "(var1,var2,...) = (v1,v2,...) or tupleset((var1,var2,...), (v1,v2,...))",
            create_expression_factory::<TupleSetExp>(),
        );
        self.register_api(
            "array",
            "[v1,v2,...] or array(v1,v2,...) object",
            create_expression_factory::<ArrayExp>(),
        );
        ...
        self.register_api(
            "echo",
            "echo(arg1,arg2,...) api",
            create_expression_factory::<EchoExp>(),
        );
        self.register_api(
            "call",
            "call(func_name,arg1,arg2,...) api",
            create_expression_factory::<CallExp>(),
        );
        self.register_api(
            "return",
            "return([val]) api",
            create_expression_factory::<ReturnExp>(),
        );
        self.register_api(
            "redirect",
            "redirect(arg1,arg2,...) api",
            create_expression_factory::<RedirectExp>(),
        );

        self.register_api(
            "bool",
            "bool(v) api",
            create_expression_factory::<BoolExp>(),
        );
        self.register_api(
            "char",
            "char(v) api",
            create_expression_factory::<CharExp>(),
        );
        self.register_api(
            "sbyte",
            "sbyte(v) api",
            create_expression_factory::<SbyteExp>(),
        );
        self.register_api(
            "ubyte",
            "ubyte(v) api",
            create_expression_factory::<UbyteExp>(),
        );
        self.register_api(
            "short",
            "short(v) api",
            create_expression_factory::<ShortExp>(),
        );
        self.register_api(
            "ushort",
            "ushort(v) api",
            create_expression_factory::<UshortExp>(),
        );
        self.register_api("int", "int(v) api", create_expression_factory::<IntExp>());
        self.register_api(
            "uint",
            "uint(v) api",
            create_expression_factory::<UintExp>(),
        );
        self.register_api(
            "long",
            "long(v) api",
            create_expression_factory::<LongExp>(),
        );
        self.register_api(
            "ulong",
            "ulong(v) api",
            create_expression_factory::<UlongExp>(),
        );
        self.register_api(
            "float",
            "float(v) api",
            create_expression_factory::<FloatExp>(),
        );
        self.register_api(
            "double",
            "double(v) api",
            create_expression_factory::<DoubleExp>(),
        );
        self.register_api(
            "tostring",
            "tostring(v) api",
            create_expression_factory::<ToStringExp>(),
        );
        self.register_api(
            "gettypestr",
            "gettypestr(v) api",
            create_expression_factory::<GetTypeStrExp>(),
        );
        self.register_api(
            "ftoi",
            "ftoi(v) api",
            create_expression_factory::<FtoiExp>(),
        );
        self.register_api(
            "itof",
            "itof(v) api",
            create_expression_factory::<ItofExp>(),
        );
        self.register_api(
            "ftou",
            "ftou(v) api",
            create_expression_factory::<FtouExp>(),
        );
        self.register_api(
            "utof",
            "utof(v) api",
            create_expression_factory::<UtofExp>(),
        );
        self.register_api(
            "dtol",
            "dtol(v) api",
            create_expression_factory::<DtolExp>(),
        );
        self.register_api(
            "ltod",
            "ltod(v) api",
            create_expression_factory::<LtodExp>(),
        );
        self.register_api(
            "dtou",
            "dtou(v) api",
            create_expression_factory::<DtouExp>(),
        );
        self.register_api(
            "utod",
            "utod(v) api",
            create_expression_factory::<UtodExp>(),
        );
        self.register_api(
            "isnull",
            "isnull(v) api",
            create_expression_factory::<IsNullExp>(),
        );
        self.register_api("null", "null() api", create_expression_factory::<NullExp>());
        self.register_api(
            "isarray",
            "isarray(v) api",
            create_expression_factory::<IsArrayExp>(),
        );
        self.register_api(
            "ishashmap",
            "ishashmap(v) api",
            create_expression_factory::<IsHashmapExp>(),
        );
        self.register_api(
            "isdeque",
            "isdeque(v) api",
            create_expression_factory::<IsDequeExp>(),
        );
        self.register_api(
            "isobject",
            "isobject(v) api",
            create_expression_factory::<IsObjectExp>(),
        );
        self.register_api(
            "isstring",
            "isstring(v) api",
            create_expression_factory::<IsStringExp>(),
        );
        self.register_api(
            "isbool",
            "isbool(v) api",
            create_expression_factory::<IsBoolExp>(),
        );
        self.register_api(
            "ischar",
            "ischar(v) api",
            create_expression_factory::<IsCharExp>(),
        );
        self.register_api(
            "isinteger",
            "isinteger(v) api",
            create_expression_factory::<IsIntegerExp>(),
        );
        self.register_api(
            "issigned",
            "issigned(v) api",
            create_expression_factory::<IsSignedExp>(),
        );
        self.register_api(
            "isunsigned",
            "isunsigned(v) api",
            create_expression_factory::<IsUnsignedExp>(),
        );
        self.register_api(
            "isinteger128",
            "isinteger128(v) api",
            create_expression_factory::<IsInteger128Exp>(),
        );
        self.register_api(
            "issigned128",
            "issigned128(v) api",
            create_expression_factory::<IsSigned128Exp>(),
        );
        self.register_api(
            "isunsigned128",
            "isunsigned128(v) api",
            create_expression_factory::<IsUnsigned128Exp>(),
        );
        self.register_api(
            "isfloat",
            "isfloat(v) api",
            create_expression_factory::<IsFloatExp>(),
        );
        self.register_api(
            "issinglefloat",
            "issinglefloat(v) api",
            create_expression_factory::<IsSingleFloatExp>(),
        );
        self.register_api(
            "isdoublefloat",
            "isdoublefloat(v) api",
            create_expression_factory::<IsDoubleFloatExp>(),
        );
        self.register_api(
            "rndint",
            "rndint(min,max) api",
            create_expression_factory::<RndIntExp>(),
        );
        self.register_api(
            "rndfloat",
            "rndfloat(min,max) api",
            create_expression_factory::<RndFloatExp>(),
        );
        self.register_api(
            "pow",
            "pow(v1,v2) api",
            create_expression_factory::<PowExp>(),
        );
        self.register_api(
            "sqrt",
            "sqrt(v) api",
            create_expression_factory::<SqrtExp>(),
        );
        self.register_api("exp", "exp(v) api", create_expression_factory::<ExpExp>());
        self.register_api(
            "exp2",
            "exp2(v) api",
            create_expression_factory::<Exp2Exp>(),
        );
        self.register_api(
            "expm1",
            "expm1(v) api",
            create_expression_factory::<ExpM1Exp>(),
        );
        self.register_api("ln", "ln(v) api", create_expression_factory::<LnExp>());
        self.register_api(
            "ln1p",
            "ln1p(v) api",
            create_expression_factory::<Ln1pExp>(),
        );
        self.register_api(
            "log",
            "log(v1,v2) api",
            create_expression_factory::<LogExp>(),
        );
        self.register_api(
            "log10",
            "log10(v) api",
            create_expression_factory::<Log10Exp>(),
        );
        self.register_api(
            "log2",
            "log2(v) api",
            create_expression_factory::<Log2Exp>(),
        );
        self.register_api(
            "max",
            "max(v1,v2,...) api",
            create_expression_factory::<MaxExp>(),
        );
        self.register_api(
            "min",
            "min(v1,v2,...) api",
            create_expression_factory::<MinExp>(),
        );
        self.register_api("abs", "abs(v) api", create_expression_factory::<AbsExp>());
        self.register_api(
            "absdiff",
            "absdiff(v) api",
            create_expression_factory::<AbsDiffExp>(),
        );
        self.register_api(
            "signum",
            "signum(v) api",
            create_expression_factory::<SigNumExp>(),
        );
        self.register_api(
            "floor",
            "floor(v) api",
            create_expression_factory::<FloorExp>(),
        );
        self.register_api(
            "ceiling",
            "ceiling(v) api",
            create_expression_factory::<CeilingExp>(),
        );
        self.register_api(
            "round",
            "round(v) api",
            create_expression_factory::<RoundExp>(),
        );
        self.register_api(
            "roundeven",
            "roundeven(v) api",
            create_expression_factory::<RoundEvenExp>(),
        );
        self.register_api(
            "floortoint",
            "floortoint(v) api",
            create_expression_factory::<FloorToIntExp>(),
        );
        self.register_api(
            "ceilingtoint",
            "ceilingtoint(v) api",
            create_expression_factory::<CeilingToIntExp>(),
        );
        self.register_api(
            "roundtoint",
            "roundtoint(v) api",
            create_expression_factory::<RoundToIntExp>(),
        );
        self.register_api(
            "roundeventoint",
            "roundeventoint(v) api",
            create_expression_factory::<RoundEvenToIntExp>(),
        );
        ...
        self.register_api(
            "sincos",
            "sincos(v) api",
            create_expression_factory::<SinCosExp>(),
        );
        self.register_api("tan", "tan(v) api", create_expression_factory::<TanExp>());
        self.register_api(
            "asin",
            "asin(v) api",
            create_expression_factory::<AsinExp>(),
        );
        self.register_api(
            "acos",
            "acos(v) api",
            create_expression_factory::<AcosExp>(),
        );
        self.register_api(
            "atan",
            "atan(v) api",
            create_expression_factory::<AtanExp>(),
        );
        self.register_api(
            "atan2",
            "atan2(v1,v2) api",
            create_expression_factory::<Atan2Exp>(),
        );
        self.register_api(
            "sinh",
            "sinh(v) api",
            create_expression_factory::<SinhExp>(),
        );
        self.register_api(
            "cosh",
            "cosh(v) api",
            create_expression_factory::<CoshExp>(),
        );
        self.register_api(
            "tanh",
            "tanh(v) api",
            create_expression_factory::<TanhExp>(),
        );
        self.register_api(
            "asinh",
            "asinh(v) api",
            create_expression_factory::<AsinhExp>(),
        );
        self.register_api(
            "acosh",
            "acosh(v) api",
            create_expression_factory::<AcoshExp>(),
        );
        self.register_api(
            "atanh",
            "atanh(v) api",
            create_expression_factory::<AtanhExp>(),
        );
        self.register_api(
            "clamp",
            "clamp(v,v1,v2) api",
            create_expression_factory::<ClampExp>(),
        );
        self.register_api(
            "clamp01",
            "clamp01(v) api",
            create_expression_factory::<Clamp01Exp>(),
        );
        self.register_api(
            "lerp",
            "lerp(a,b,t) api",
            create_expression_factory::<LerpExp>(),
        );
        self.register_api(
            "lerpunclamped",
            "lerpunclamped(a,b,t) api",
            create_expression_factory::<LerpUnclampedExp>(),
        );
        self.register_api(
            "lerpangle",
            "lerpangle(a,b,t) api",
            create_expression_factory::<LerpAngleExp>(),
        );
        self.register_api(
            "smoothstep",
            "smoothstep(from,to,t) api",
            create_expression_factory::<SmoothStepExp>(),
        );
        self.register_api(
            "approximately",
            "approximately(v1,v2) api",
            create_expression_factory::<ApproximatelyExp>(),
        );
        self.register_api(
            "ispoweroftwo",
            "ispoweroftwo(v) api",
            create_expression_factory::<IsPowerOfTwoExp>(),
        );
        self.register_api(
            "closestpoweroftwo",
            "closestpoweroftwo(v) api",
            create_expression_factory::<ClosestPowerOfTwoExp>(),
        );
        self.register_api(
            "nextpoweroftwo",
            "nextpoweroftwo(v) api",
            create_expression_factory::<NextPowerOfTwoExp>(),
        );
        self.register_api(
            "dist",
            "dist(x1,y1,x2,y2) api",
            create_expression_factory::<DistExp>(),
        );
        self.register_api(
            "distsqr",
            "distsqr(x1,y1,x2,y2) api",
            create_expression_factory::<DistSqrExp>(),
        );
        self.register_api(
            "isempty",
            "isempty(str) api",
            create_expression_factory::<IsEmptyExp>(),
        );
        self.register_api(
            "str2int",
            "str2int(str) api",
            create_expression_factory::<Str2IntExp>(),
        );
        self.register_api(
            "str2uint",
            "str2uint(str) api",
            create_expression_factory::<Str2UintExp>(),
        );
        self.register_api(
            "str2long",
            "str2long(str) api",
            create_expression_factory::<Str2LongExp>(),
        );
        self.register_api(
            "str2ulong",
            "str2ulong(str) api",
            create_expression_factory::<Str2UlongExp>(),
        );
        self.register_api(
            "str2float",
            "str2float(str) api",
            create_expression_factory::<Str2FloatExp>(),
        );
        self.register_api(
            "str2double",
            "str2double(str) api",
            create_expression_factory::<Str2DoubleExp>(),
        );
        self.register_api(
            "hex2int",
            "hex2int(str) api",
            create_expression_factory::<Hex2IntExp>(),
        );
        self.register_api(
            "hex2uint",
            "hex2uint(str) api",
            create_expression_factory::<Hex2UintExp>(),
        );
        self.register_api(
            "hex2long",
            "hex2long(str) api",
            create_expression_factory::<Hex2LongExp>(),
        );
        self.register_api(
            "hex2ulong",
            "hex2ulong(str) api",
            create_expression_factory::<Hex2UlongExp>(),
        );
        self.register_api(
            "tohex",
            "tohex(num[,bool_upper_case]) api",
            create_expression_factory::<ToHexExp>(),
        );
        self.register_api(
            "strconcat",
            "strconcat(arg1,arg2,...) api",
            create_expression_factory::<StrConcatExp>(),
        );
        self.register_api(
            "substring",
            "substring(str,start[,len]) function",
            create_expression_factory::<SubStringExp>(),
        );
        self.register_api(
            "stringappend",
            "stringappend(sb,arg1,arg2,...) api",
            create_expression_factory::<StringAppendExp>(),
        );
        self.register_api(
            "stringappendline",
            "stringappendline(sb,arg1,arg2,...) api",
            create_expression_factory::<StringAppendLineExp>(),
        );
        self.register_api(
            "stringjoin",
            "stringjoin(sep,list) api",
            create_expression_factory::<StringJoinExp>(),
        );
        self.register_api(
            "stringsplit",
            "stringsplit(str,sep_list) api",
            create_expression_factory::<StringSplitExp>(),
        );
        self.register_api(
            "stringtrim",
            "stringtrim(str) api",
            create_expression_factory::<StringTrimExp>(),
        );
        self.register_api(
            "stringtrimstart",
            "stringtrimstart(str) api",
            create_expression_factory::<StringTrimStartExp>(),
        );
        self.register_api(
            "stringtrimend",
            "stringtrimend(str) api",
            create_expression_factory::<StringTrimEndExp>(),
        );
        self.register_api(
            "stringtrimmatches",
            "stringtrimmatches(str,pattern) api",
            create_expression_factory::<StringTrimMatchesExp>(),
        );
        self.register_api(
            "stringtrimstartmatches",
            "stringtrimstartmatches(str,pattern) api",
            create_expression_factory::<StringTrimStartMatchesExp>(),
        );
        self.register_api(
            "stringtrimendmatches",
            "stringtrimendmatches(str,pattern) api",
            create_expression_factory::<StringTrimEndMatchesExp>(),
        );
        self.register_api(
            "stringtolower",
            "stringtolower(str) api",
            create_expression_factory::<StringToLowerExp>(),
        );
        self.register_api(
            "stringtoupper",
            "stringtoupper(str) api",
            create_expression_factory::<StringToUpperExp>(),
        );
        self.register_api(
            "stringreplace",
            "stringreplace(str,key,rep_str) api",
            create_expression_factory::<StringReplaceExp>(),
        );
        self.register_api(
            "stringreplacechar",
            "stringreplacechar(str,key,char_as_str) api",
            create_expression_factory::<StringReplaceCharExp>(),
        );
        self.register_api(
            "makestring",
            "makestring(char1_as_str_or_int,char2_as_str_or_int,...) api",
            create_expression_factory::<MakeStringExp>(),
        );
        self.register_api(
            "stringcontains",
            "stringcontains(str,str_or_list_1,str_or_list_2,...) api",
            create_expression_factory::<StringContainsExp>(),
        );
        self.register_api(
            "stringnotcontains",
            "stringnotcontains(str,str_or_list_1,str_or_list_2,...) api",
            create_expression_factory::<StringNotContainsExp>(),
        );
        self.register_api(
            "stringcontainsany",
            "stringcontainsany(str,str_or_list_1,str_or_list_2,...) api",
            create_expression_factory::<StringContainsAnyExp>(),
        );
        self.register_api(
            "stringnotcontainsany",
            "stringnotcontainsany(str,str_or_list_1,str_or_list_2,...) api",
            create_expression_factory::<StringNotContainsAnyExp>(),
        );
        ...
        self.register_api(
            "listsize",
            "listsize(list) api",
            create_expression_factory::<ListSizeExp>(),
        );
        self.register_api(
            "listget",
            "listget(list,index[,defval]) api",
            create_expression_factory::<ListGetExp>(),
        );
        self.register_api(
            "listset",
            "listset(list,index,val) api",
            create_expression_factory::<ListSetExp>(),
        );
        self.register_api(
            "listindexof",
            "listindexof(list,val) api",
            create_expression_factory::<ListIndexOfExp>(),
        );
        self.register_api(
            "listadd",
            "listadd(list,val) api",
            create_expression_factory::<ListAddExp>(),
        );
        self.register_api(
            "listremove",
            "listremove(list,val) api",
            create_expression_factory::<ListRemoveExp>(),
        );
        self.register_api(
            "listinsert",
            "listinsert(list,index,val) api",
            create_expression_factory::<ListInsertExp>(),
        );
        self.register_api(
            "listremoveat",
            "listremoveat(list,index) api",
            create_expression_factory::<ListRemoveAtExp>(),
        );
        self.register_api(
            "listclear",
            "listclear(list) api",
            create_expression_factory::<ListClearExp>(),
        );
        self.register_api(
            "listsplit",
            "listsplit(list,ct) api, return list of list",
            create_expression_factory::<ListSplitExp>(),
        );
        self.register_api(
            "hashtablesize",
            "hashtablesize(hash) api",
            create_expression_factory::<HashtableSizeExp>(),
        );
        self.register_api(
            "hashtableget",
            "hashtableget(hash,key[,defval]) api",
            create_expression_factory::<HashtableGetExp>(),
        );
        self.register_api(
            "hashtableset",
            "hashtableset(hash,key,val) api",
            create_expression_factory::<HashtableSetExp>(),
        );
        self.register_api(
            "hashtableadd",
            "hashtableadd(hash,key,val) api",
            create_expression_factory::<HashtableAddExp>(),
        );
        self.register_api(
            "hashtableremove",
            "hashtableremove(hash,key) api",
            create_expression_factory::<HashtableRemoveExp>(),
        );
        self.register_api(
            "hashtableclear",
            "hashtableclear(hash) api",
            create_expression_factory::<HashtableClearExp>(),
        );
        self.register_api(
            "hashtablekeys",
            "hashtablekeys(hash) api",
            create_expression_factory::<HashtableKeysExp>(),
        );
        self.register_api(
            "hashtablevalues",
            "hashtablevalues(hash) api",
            create_expression_factory::<HashtableValuesExp>(),
        );
        self.register_api(
            "listhashtable",
            "listhashtable(hash) api, return list of pair",
            create_expression_factory::<ListHashtableExp>(),
        );
        self.register_api(
            "hashtablesplit",
            "hashtablesplit(hash,ct) api, return list of hashtable",
            create_expression_factory::<HashtableSplitExp>(),
        );
        self.register_api(
            "deque",
            "deque(v1,v2,...) object",
            create_expression_factory::<DequeExp>(),
        );
        self.register_api(
            "dequesize",
            "dequesize(deque) api",
            create_expression_factory::<DequeSizeExp>(),
        );
        self.register_api(
            "front",
            "front(deque) api",
            create_expression_factory::<DequeFrontExp>(),
        );
        self.register_api(
            "back",
            "back(deque) api",
            create_expression_factory::<DequeBackExp>(),
        );
        self.register_api(
            "pushback",
            "pushback(deque,v) api",
            create_expression_factory::<DequePushBackExp>(),
        );
        self.register_api(
            "popback",
            "popback(deque) api",
            create_expression_factory::<DequePopBackExp>(),
        );
        self.register_api(
            "pushfront",
            "pushfront(deque,v) api",
            create_expression_factory::<DequePushFrontExp>(),
        );
        self.register_api(
            "popfront",
            "popfront(deque) api",
            create_expression_factory::<DequePopFrontExp>(),
        );
        self.register_api(
            "dequeclear",
            "dequeclear(queue) api",
            create_expression_factory::<DequeClearExp>(),
        );

        self.register_api(
            "format",
            "format(fmt,arg1,arg2,...) api",
            create_expression_factory::<FormatExp>(),
        );
        self.register_api(
            "objectnew",
            "objectnew(name)",
            create_expression_factory::<ObjectNewExp>(),
        );
        self.register_api(
            "objectrelease",
            "objectrelease(obj)",
            create_expression_factory::<ObjectReleaseExp>(),
        );
        self.register_api(
            "objnewwithstr",
            "objnewwithstr(name_str)",
            create_expression_factory::<ObjNewWithStrExp>(),
        );
        self.register_api(
            "objgetclassname",
            "objgetclassname(obj)",
            create_expression_factory::<ObjGetClassNameExp>(),
        );
        self.register_api(
            "objgetdispid",
            "objgetdispid(obj,member_name)",
            create_expression_factory::<ObjGetDispIdExp>(),
        );
        self.register_api(
            "collectioncall",
            "collectioncall api, fn implementation, using object syntax",
            create_expression_factory::<CollectionCallExp>(),
        );
        self.register_api(
            "collectionset",
            "collectionset api, fn implementation, using object syntax",
            create_expression_factory::<CollectionSetExp>(),
        );
        self.register_api(
            "collectionget",
            "collectionget api, fn implementation, using object syntax",
            create_expression_factory::<CollectionGetExp>(),
        );
        self.register_api(
            "objectcall",
            "objectcall api, fn implementation, using object syntax",
            create_expression_factory::<ObjectCallExp>(),
        );
        self.register_api(
            "objectset",
            "objectset api, fn implementation, using object syntax",
            create_expression_factory::<ObjectSetExp>(),
        );
        self.register_api(
            "objectget",
            "objectget api, fn implementation, using object syntax",
            create_expression_factory::<ObjectGetExp>(),
        );
    ...
    pub fn set_global_variable(&mut self, v: &str, val: DslCalculatorValue) -> bool {
        if let Some(callback) = self.on_set_variable.as_deref_mut() {
            ...
            self.m_named_global_variable_indexes
                .insert(String::from(v), ix as i32);
    ...
    pub fn unhold_object(&mut self, obj: &ObjectRcCell) -> bool {
        return self
            .m_objects
            .remove(&obj.borrow().get_object_id())
            .is_some();
    }
    ...
    pub fn check_func_xrefs(&self) {
        for func in self.m_func_calls.iter() {
            ...
            //error
            self.error(&format!(
                "DslCalculator error, unknown func '{}', {} line {}",
                func.get_id(),
    ...
    fn convert_member(&self, mut p: SyntaxComponent, param_class: i32) -> SyntaxComponent {
        if let SyntaxComponent::Value(pvd) = &mut p {
            if pvd.is_id()
                && (param_class == dsl::PARAM_CLASS_PERIOD
    ...
    pub fn load_dsl_info(cell: &Rc<DslCalculatorCell<'a>>, info: &SyntaxComponent) {
        ...
        cell.borrow_mut()
            .m_funcs
            .insert(String::from(id), Rc::new(func_info));
    }
    ...
    pub fn load_dsl_func_with_args(
        cell: &Rc<DslCalculatorCell<'a>>,
        func: &str,
        param_names: Option<&'a Vec<String>>,
        dsl_func: &FunctionData,
    ) {
        ...
        cell.borrow_mut()
            .m_funcs
            .insert(String::from(func), Rc::new(func_info));
    }
    pub fn load_dsl_statements(
        cell: &Rc<DslCalculatorCell<'a>>,
        statements: &Vec<SyntaxComponent>,
        exps: &mut Vec<ExpressionBox<'a>>,
    ) {
    ...
    pub fn calc_n(
        cell: &Rc<DslCalculatorCell<'a>>,
        func: &str,
        args: &mut Vec<DslCalculatorValue>,
    ) -> DslCalculatorValue {
        ...
        if let Some(func_info) = func_info_opt {
            ...
            //error
            cell.borrow()
                .error(&format!("DslCalculator error, unknown func {}", func));
            return DslCalculatorValue::Null;
        }
        cell.borrow_mut().local_stack_pop();
        return ret;
    }
    pub fn calc_in_current_context(
        cell: &Rc<DslCalculatorCell<'a>>,
        func: &str,
    ) -> DslCalculatorValue {
        let mut ret = DslCalculatorValue::Null;
    ...
    pub fn load_value_syntax(
        cell: &Rc<DslCalculatorCell<'a>>,
        value_data: &ValueData,
    ) -> Option<ExpressionBox<'a>> {
        ...
        if id_type == dsl::ID_TOKEN {
            ...
            if let Some(mut p) = api {
                ...
                if !p.load_function_syntax(fd, cell.clone()) {
                    //error
                    cell.borrow().error(&format!(
                        "DslCalculator error, {0} line {1}",
                        value_data.to_script_string(false, &dsl::DEFAULT_DELIM),
                        value_data.get_line()
                    ));
                }
                return Some(p);
                ...
                AbstractExpression::load_value_syntax(
                    &mut const_exp,
                    value_data.clone(),
                    cell.clone(),
                );
    ...
    pub fn load_function_syntax(
        cell: &Rc<DslCalculatorCell<'a>>,
        func_data: &FunctionData,
    ) -> Option<ExpressionBox<'a>> {
        if func_data.have_param() {
            if !func_data.have_id()
                && !func_data.is_high_order()
                && (func_data.get_param_class() == dsl::PARAM_CLASS_PARENTHESES
                    || func_data.get_param_class() == dsl::PARAM_CLASS_BRACKET)
            {
                match func_data.get_param_class() {
                    dsl::PARAM_CLASS_PARENTHESES => {
                        ...
                        if num == 1 {
                            ...
                            AbstractExpression::load_function_syntax(
                                &mut exp,
                                func_data.clone(),
                                cell.clone(),
                            );
                ...
                if op == "=" {
                    ...
                    if let Some(syn) = func_data.get_param(0) {
                        if let SyntaxComponent::Function(inner_call) = syn {
                                ...
                                || inner_param_class == dsl::PARAM_CLASS_BRACKET
                            {
                                ...
                                if inner_param_class == dsl::PARAM_CLASS_PERIOD {
                                    new_call.set_name(Box::new(ValueData::from_string_type(
                                        String::from("objectset"),
                                        dsl::ID_TOKEN,
                                    )));
                                } else {
                                    new_call.set_name(Box::new(ValueData::from_string_type(
                                        String::from("collectionset"),
                                        dsl::ID_TOKEN,
                                    )));
                                }
                                ...
                                if inner_call.is_high_order() {
                                    ...
                                    if let Some(param) = inner_call.get_param(0) {
                                        new_call.add_syntax_component_param(
                                            cell.borrow().convert_member(
                                                param.clone(),
                                                inner_call.get_param_class(),
                                            ),
                                        );
                                    }
                                ...
                                return Self::load_syntax_component(
                                    cell,
                                    &SyntaxComponent::Function(new_call),
                                );
                            } else if inner_param_class == dsl::PARAM_CLASS_PARENTHESES
                                && !inner_call.have_id()
                            {
                                //(a,b,c) = val
                    ...
                    if func_data.is_high_order() {
                        if let Some(inner_call) = func_data.lower_order_function() {
                            let inner_param_class = inner_call.get_param_class();
                            if param_class == dsl::PARAM_CLASS_PARENTHESES
                                && (inner_param_class == dsl::PARAM_CLASS_PERIOD
                                    || inner_param_class == dsl::PARAM_CLASS_BRACKET)
                            {
                    ...
                    if param_class == dsl::PARAM_CLASS_PERIOD
                        || param_class == dsl::PARAM_CLASS_BRACKET
                    {
                        ...
                        if param_class == dsl::PARAM_CLASS_PERIOD {
                            new_call.set_name(Box::new(ValueData::from_string_type(
                                String::from("objectget"),
                                dsl::ID_TOKEN,
                            )));
                        } else {
                            new_call.set_name(Box::new(ValueData::from_string_type(
                                String::from("collectionget"),
                                dsl::ID_TOKEN,
                            )));
                        }
                        new_call.set_parentheses_param_class();
                        if func_data.is_high_order() {
                            ...
                            if let Some(param) = func_data.get_param(0) {
                                new_call.add_syntax_component_param(
                                    cell.borrow()
                                        .convert_member(param.clone(), func_data.get_param_class()),
                                );
                            }
                        } else {
    ...
    pub fn load_statement_syntax(
        ...
        if let Some(mut ret) = api {
            ...
            if let Some(fd) =
                DslSyntaxTransformer::try_transform_command_line_like_syntax(statement_data)
            {
            ...
            cell.borrow().error(&format!(
                "DslCalculator error, {0} line {1}",
                statement_data.to_script_string(false, &dsl::DEFAULT_DELIM),
                statement_data.get_line()
            ));
            return None;
        }
        // We enable the function to be called before it is defined, so failover is done first
    ...
    pub fn load_value_or_function(
        cell: &Rc<DslCalculatorCell<'a>>,
        comp: &ValueOrFunction,
    ) -> Option<ExpressionBox<'a>> {
        match comp {
            ValueOrFunction::Value(value_data) => {
                return Self::load_value_syntax(cell, value_data);
...
impl DslSyntaxTransformer {
    pub fn try_transform_command_line_like_syntax(
        statement_data: &StatementData,
    ) -> Option<FunctionData> {

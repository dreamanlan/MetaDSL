...
use crate::dsl_expression::dsl_calculator::DslCalculator;
...
mod impl_mod {
    ...
    use crate::dsl_expression::dsl_calculator::{
        DslCalculator, DslCalculatorCell, DslCalculatorValue, IObjectDispatch,
    };
    use metadsl::dsl::{DslFile, DslLogDelegationBox};
    use metadsl_macros::{add_object_dispatch_fields, impl_object_dispatch};
    ...
    pub fn interactive_execution(
        calculator: &Rc<DslCalculatorCell>,
        log_callback: &DslLogDelegationBox,
    ) {
...
fn main() {
    ...
    let calculator = DslCalculator::new_cell();
    {
        ...
        calculator.borrow_mut().register_class(
            "ObjectTest",
            "test object api",
            create_class_factory::<impl_mod::ObjectTest>(),
        );
        let mut dsl_file = DslFile::new();
        let content = r#"
            script(main){
